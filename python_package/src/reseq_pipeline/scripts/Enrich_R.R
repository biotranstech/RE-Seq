                         ...
                         method)
{
      ...
      if (max1 <= 3 & nrow(go_enrich_df) <= 10){
      	smart_ggsave(P1,file=paste0(R_Enrichment_path,"/",i,"_",method,"_",up_down,"_GO1.pdf"),size_num=6)
      	smart_ggsave(P1,file=paste0(R_Enrichment_path,"/",i,"_",method,"_",up_down,"_GO1.png"),size_num=6)
      }else{
      ...
      P2 = ggplot(data=go_enrich_df, aes(x=type_order,y=GeneNumber, fill=type)) + 
            ...
            theme_style()
      if (max1 <= 3 & nrow(go_enrich_df) <= 10){
        smart_ggsave(P2,file=paste0(R_Enrichment_path,"/",i,"_",method,"_",up_down,"_GO2.pdf"),size_num=6)
        smart_ggsave(P2,file=paste0(R_Enrichment_path,"/",i,"_",method,"_",up_down,"_GO2.png"),size_num=6)
      }else{
      ...
      #1、KEGG富集
      #kk <- enrichKEGG(gene = gene,keyType = "kegg",organism= database2, qvalueCutoff = 1, pvalueCutoff=1)
      t_gene = paste0("/mnt/sda/project/yjc1/pipeline/01_RE-seq/KEGG_data/", database2, "_kegg_gene_pathway.rds")
      t_name = paste0("/mnt/sda/project/yjc1/pipeline/01_RE-seq/KEGG_data/", database2, "_kegg_pathway_list.rds")
      term2gene <- readRDS(t_gene)
      term2name <- readRDS(t_name)
      kk <- enricher(
            gene = gene,
	    TERM2GENE = term2gene,
	    TERM2NAME = term2name,
      ...
      if (max1 <= 3 & nrow(hh) <= 10){
        smart_ggsave(P3,file=paste0(R_Enrichment_path,"/",i,"_",method,"_",up_down,"_KK1.pdf"),size_num=6)
        smart_ggsave(P3,file=paste0(R_Enrichment_path,"/",i,"_",method,"_",up_down,"_KK1.png"),size_num=6)
      }else{
      ...
      P4 = ggplot(hh,aes(y=Description,x=Count))+
            ...
            theme_bw()+ theme_style()
      if (max1 <= 3 & nrow(hh) <= 10){
        smart_ggsave(P4,file=paste0(R_Enrichment_path,"/",i,"_",method,"_",up_down,"_KK2.pdf"),size_num=6)
        smart_ggsave(P4,file=paste0(R_Enrichment_path,"/",i,"_",method,"_",up_down,"_KK2.png"),size_num=6)
      }else{
